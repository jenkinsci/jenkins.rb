...
module Jenkins
  class Job
    class ConfigBuilder
      ...
      MissingGemfile  = Class.new(StandardError)
      ...
      def initialize(job_type = :ruby, &block)
        ...
        raise MissingGemfile if ruby_or_rails_project_without_gemfile?
      end
      ...
      def ruby_or_rails_project_without_gemfile?
        job_type =~ /ruby|rails/ && !File.exists?("Gemfile")
      end
    end
  end
end
